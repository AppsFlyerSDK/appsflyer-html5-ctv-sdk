...
const PLATFORM_MAPPING = [Samsung, LG, Vizio, Vidaa, Xbox];
...
class AppsFlyerSDK {
  ...
  setPlatformInstance() {
    try {
      ...
      const availablePlatforms = [
        window.tizen && tizen.application,
        window.webOS && webOS.fetchAppInfo && webOS.platform.tv,
        window.VIZIO,
        window.VIDAA,
        window.xbox,
      ];
      const plafromIndex = availablePlatforms
        .map((p) => isDefined(p))
        .findIndex((p) => p == true);
      if (plafromIndex !== -1) {
  ...
  async init(config) {
    return new Promise(async (resolve, reject) => {
      ...
      if (this.isSDKValid()) {
        ...
        const isVersionValid = await this.validateOSVersion(
          platformData.platform,
          platformData.payload.device_os_version
        );
        if (isVersionValid) {
          try {
            await this.appsflyerInstance.init(
              config,
              platformData,
              platformLogs
            );
  ...
  start() {
    return this.isSDKValid()
      ? this.appsflyerInstance.start()
      : new Error(INVALID_SDK);
  }

  logEvent(eventName, eventValue) {
    return this.isSDKValid()
      ? this.appsflyerInstance.logEvent(eventName, eventValue)
      : new Error(INVALID_SDK);
  }

  setCustomPayload(payload) {
    return this.isSDKValid()
      ? this.appsflyerInstance.setCustomPayload(payload)
      : new Error(INVALID_SDK);
  }

  setCustomerUserId(userId) {
    return this.isSDKValid()
      ? this.appsflyerInstance.setCustomerUserId(userId)
      : new Error(INVALID_SDK);
  }

  async validateOSVersion(os, osVersion) {
    if (
      (os == Platform.Tizen || os == Platform.Webos) &&
      isVersionGreaterThanOrEqualTo(osVersion, MINIMUM_OS_VERSION_SUPPORTED)
    ) {
      return true;
    } else {
